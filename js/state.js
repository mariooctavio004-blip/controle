...
const defaultState = {
    ...
    /* Datas completas correspondentes a state.days. */
    dayKeys: [],
...
function normalizeLoadedState(savedState) {
    ...
    normalized.dayKeys = Array.isArray(normalized.dayKeys)
        ? normalized.dayKeys
        : [];
    ...
    normalized.importedFiles = Array.isArray(normalized.importedFiles)
        ? normalized.importedFiles
        : [];
...
function ensureImportedWorkbookState() {
    if (!Array.isArray(state.importedFiles)) state.importedFiles = [];
}
