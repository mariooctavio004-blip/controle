...
async function loadLocalWorkbooksIntoImportedData(options = {}) {
    ...
    for (const record of records) {
        try {
            const workbook = XLSX.read(record.buffer, { type: "array", cellDates: true });
            const result = processWorkbook(workbook, record.fileName || "", { source: "local", fileId: record.id });
            const file = ensureImportedFilesState().find(item => item.id === record.id);
            if (file) {
                file.sheets = result.recognizedSheets;
                file.panels = [...new Set(result.recognizedSheets.map(item => item.key))];
                file.ignoredSheets = [...new Set([...(result.ignored || []), ...(result.hidden || [])])];
                file.extractionErrors = result.errors || [];
                file.lastSync = new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });
            }
            if (result.recognized.length) count++;
        } catch (error) {
            console.warn(`Não foi possível restaurar ${record.fileName || "planilha"}:`, error);
        }
    }
