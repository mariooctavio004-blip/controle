...
async function readLocalWorkbookBuffers(options = {}) {
    const { activeOnly = true, purgeOrphans = true } = options;
    const db = await openLocalWorkbookDatabase();
    if (!db) return [];
    ...
    const activeFiles = Array.isArray(state.importedFiles)
        ? state.importedFiles.filter(item => item && item.source === "local" && item.id)
        : [];

    const activeIds = new Set(activeFiles.map(item => String(item.id)));
    const availableRows = rows.filter(row => row && activeIds.has(String(row.id)));
    const availableIds = new Set(availableRows.map(row => String(row.id)));

    /*
     * Metadados vindos do Supabase não significam que o arquivo existe
     * neste navegador. Sem o buffer local, a planilha não pode ser tratada
     * como fonte ativa nem aparecer como disponível.
     */
    state.importedFiles = (state.importedFiles || []).filter(item => {
        if (!item || item.source !== "local") return true;
        return availableIds.has(String(item.id));
    });

    /*
     * Remove buffers órfãos que ficaram no IndexedDB após versões antigas,
     * falhas de desvinculação ou troca de estado. Esses arquivos eram a
     * origem dos dados que apareciam "do nada" ao clicar em Atualizar.
     */
    if (purgeOrphans) {
        const orphanRows = rows.filter(row => row && !activeIds.has(String(row.id)));
        for (const orphan of orphanRows) {
            try {
                await deleteLocalWorkbookBuffer(orphan.id);
            } catch (error) {
                console.warn("Não foi possível remover um arquivo órfão do cache:", error);
            }
        }
    }
...
/* ============================================================
   CAMADA PROFISSIONAL DE CONSOLIDAÇÃO E RASTREABILIDADE
   - mantém todos os arquivos locais ativos;
   - usa datas completas internamente;
   - combina meses diferentes;
   - lista somente abas efetivamente processadas;
   - impede dados sem origem ao atualizar.
============================================================ */
...
function getImportedSheetEntries(key) {
    ...
    if (isMatrix(value)) {
        return [{
            ...
            matrix: value,
            source: "url",
            fileName: state?.sharepoint?.[key]?.name || "URL conectada",
            fileId: `url-${key}`
        }];
    }
    if (!Array.isArray(value)) return [];
    return value
        .filter(item => item && isMatrix(item.matrix) && item.matrix.length)
        .map(item => ({
...
function addImportedSheet(key, matrix, sheetName = "", sourceInfo = {}) {
    if (!EXCEL_PANEL_KEYS.includes(key)) return;
    if (!isMatrix(matrix) || !matrix.length) return;
    const current = importedData[key];
    if (!Array.isArray(current) || isMatrix(current)) {
        importedData[key] = isMatrix(current) && current.length
            ? [{
                sheetName: EXCEL_PANEL_LABELS[key],
                matrix: current,
                source: "url",
    ...
    importedData[key].push({
        sheetName,
        matrix,
        source: sourceInfo.source || "local",
...
function getImportedMatrices(key) {
    return getImportedSheetEntries(key).map(item => item.matrix);
}

function classifyAuthorizedSheetName(name) {
    const normalized = normalizeName(name);
    const exact = {
        "CAMPO": "campo",
        "OPERACOES EM CAMPO": "campo",
        "OPERACAO EM CAMPO": "campo",
        "ABASTECIMENTO": "abastecimento",
        "DIARIO DE CAMPO": "diario",
        "DIARIO DE CAMPO MANEJO REBANHO": "diario",
        "DIVERGENCIAS": "divergencias",
        "DIVERGENCIA": "divergencias",
        "MENSAL": "mensal",
        "MAPA MENSAL": "mensal",
        "MAPA MENSAL DO REBANHO": "mensal",
        "FECHAMENTO MENSAL": "mensal"
    };
    if (exact[normalized]) return exact[normalized];
    if (normalized.startsWith("DIARIO DE CAMPO")) return "diario";
    if (normalized.startsWith("DIVERGENC")) return "divergencias";
    if (normalized.startsWith("ABASTECIMENTO")) return "abastecimento";
    if (normalized.startsWith("OPERACOES EM CAMPO")) return "campo";
    if (normalized.startsWith("MAPA MENSAL") || normalized.startsWith("FECHAMENTO MENSAL")) return "mensal";
    return null;
}

function findLikelyDateRangeInDailyParsed(parsed, fileName = "") {
    if (!parsed?.days?.length) return { start: "", end: "", keys: [] };
    const yearMatch = String(fileName).match(/(?:19|20)\d{2}/);
    const fallbackYear = yearMatch
        ? Number(yearMatch[0])
        : (isoToDate(state?.filterEnd)?.getFullYear() || isoToDate(state?.filterStart)?.getFullYear() || new Date().getFullYear());
    let previousMonth = null;
    let year = fallbackYear;
    const keys = parsed.days.map(label => {
        ...
        if (!match[3] && previousMonth !== null && month < previousMonth && previousMonth === 12) itemYear = ++year;
        previousMonth = month;
        return `${itemYear}-${String(month).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
    }).filter(Boolean);
    keys.sort();
    return { start: keys[0] || "", end: keys[keys.length - 1] || "", keys };
}

function validateAuthorizedSheet(key, matrix, context = {}) {
    try {
        if (key === "campo" || key === "abastecimento" || key === "diario") {
            const parsed = parseStatusSheet(matrix);
            if (!parsed?.rows?.length || !parsed?.days?.length) {
                return { valid: false, reason: "Cabeçalho de fazenda/datas não reconhecido." };
            }
            const range = findLikelyDateRangeInDailyParsed(parsed, context.fileName);
            return {
                valid: true,
                records: parsed.rows.length * parsed.days.length,
                farms: parsed.rows.length,
                dateStart: range.start,
                dateEnd: range.end
            };
        }
        if (key === "divergencias") {
            const hasFarm = matrix.some(row => (row || []).some(cell => normalizeName(cell) === "FAZENDA"));
            const hasDate = matrix.some(row => (row || []).some(cell => ["DATA","REFERENCIA","REFERÊNCIA"].includes(String(cell || "").trim().toUpperCase())));
            if (!hasFarm || !hasDate) return { valid: false, reason: "Colunas Fazenda e Data não foram localizadas." };
            const rows = matrix.filter(row => row && row.some(cell => String(cell || "").trim())).length;
            return { valid: true, records: Math.max(0, rows - 1), farms: 0, dateStart: "", dateEnd: "" };
        }
        if (key === "mensal") {
            const hasFarm = matrix.some(row => (row || []).some(cell => normalizeName(cell) === "FAZENDA"));
            if (!hasFarm) return { valid: false, reason: "Coluna Fazenda não foi localizada." };
            return { valid: true, records: Math.max(0, matrix.length - 1), farms: 0, dateStart: "", dateEnd: "" };
        }
    } catch (error) {
        return { valid: false, reason: error?.message || "Erro de validação." };
    }
    return { valid: false, reason: "Tipo de aba não suportado." };
}

function processWorkbook(workbook, fallbackName = "", sourceInfo = {}) {
    ...
    workbook.SheetNames.forEach((sheetName, sheetIndex) => {
        if (Number(metadata[sheetIndex]?.Hidden || 0) !== 0) {
            hidden.push(sheetName);
            return;
        }
        const key = classifyAuthorizedSheetName(sheetName);
        if (!key) {
            ignored.push(sheetName);
            return;
        }
        const matrix = sheetToMatrix(workbook.Sheets[sheetName]);
        const validation = validateAuthorizedSheet(key, matrix, { fileName: fallbackName });
        if (!validation.valid) {
            errors.push({ sheetName, key, reason: validation.reason });
            return;
        }
        addImportedSheet(key, matrix, sheetName, {
            source: sourceInfo.source || "local",
        ...
        recognizedSheets.push({
            name: sheetName,
            key,
            records: validation.records || 0,
            farms: validation.farms || 0,
    ...
    return { recognized, recognizedSheets, ignored, hidden, errors };
}

function registerImportedLocalFile(fileName, sheets = [], ignoredSheets = [], extra = {}) {
    const files = ensureImportedFilesState();
    const normalizedFileName = String(fileName || "Planilha sem nome").trim();
    const normalizedSheets = (Array.isArray(sheets) ? sheets : [])
        .filter(item => item && item.status !== "error" && EXCEL_PANEL_KEYS.includes(item.key))
        .map(item => ({
            name: String(item.name || item.sheetName || "").trim(),
            key: item.key,
            records: Number(item.records || 0),
            farms: Number(item.farms || 0),
            dateStart: String(item.dateStart || ""),
            dateEnd: String(item.dateEnd || ""),
            status: "processed"
        }));
    const existingIndex = files.findIndex(item => normalizeName(item?.fileName) === normalizeName(normalizedFileName));
    const id = extra.id || (existingIndex >= 0 ? files[existingIndex].id : `local-${Date.now()}-${Math.random().toString(36).slice(2,8)}`);
    const record = {
        id,
        source: "local",
        fileName: normalizedFileName,
        sheets: normalizedSheets,
        panels: [...new Set(normalizedSheets.map(item => item.key))],
        ignoredSheets: [...new Set(ignoredSheets || [])],
        extractionErrors: Array.isArray(extra.errors) ? extra.errors : [],
        importedAt: existingIndex >= 0 ? files[existingIndex].importedAt : new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        lastSync: new Date().toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })
    };
    if (existingIndex >= 0) files[existingIndex] = record;
    else files.push(record);
    return record;
}

function formatSourcePeriod(sheet) {
    if (!sheet?.dateStart && !sheet?.dateEnd) return "";
    const show = iso => {
        const d = isoToDate(iso);
        return d ? `${String(d.getDate()).padStart(2,"0")}/${String(d.getMonth()+1).padStart(2,"0")}/${d.getFullYear()}` : iso;
    };
    return sheet.dateStart === sheet.dateEnd
        ? show(sheet.dateStart)
        : `${show(sheet.dateStart)} a ${show(sheet.dateEnd)}`;
}
...
function renderImportedWorkbookStatus() {
    ...
    if (!indicator || !list) return;
    const localFiles = getImportedLocalFiles();
    const urlConnections = EXCEL_PANEL_KEYS.map(key => ({ key, ...(state.sharepoint?.[key] || {}) })).filter(item => item.url);
    const totalSources = localFiles.length + urlConnections.length;
    if (!totalSources) {
        indicator.textContent = "Nenhuma planilha conectada";
        list.textContent = "Selecione ou conecte uma planilha";
    ...
    indicator.textContent = `🟢 ${totalSources} planilha(s) disponível(is)`;
    const localHTML = localFiles.map(file => {
        const sheets = (file.sheets || []).filter(sheet => sheet.status === "processed");
        const sheetHTML = sheets.length
            ? sheets.map(sheet => {
                const period = formatSourcePeriod(sheet);
                const details = [
                    `${Number(sheet.records || 0)} registro(s)`,
                    period ? `período ${period}` : ""
                ].filter(Boolean).join(" • ");
                return `<span class="processed-sheet-line">✅ ${escapeHtml(sheet.name)} → ${escapeHtml(EXCEL_PANEL_LABELS[sheet.key])}${details ? `<small>${escapeHtml(details)}</small>` : ""}</span>`;
            }).join("")
            : `<span class="processed-sheet-line">⚠️ Nenhuma aba produziu dados válidos</span>`;
        return `<div class="connected-item connected-item-local">
            <span>📄</span>
            <div class="connected-item-content">
                <strong>${escapeHtml(file.fileName)}</strong>
                <div class="processed-sheet-list">${sheetHTML}</div>
                ${(file.ignoredSheets || []).length ? `<small>${file.ignoredSheets.length} aba(s) ignorada(s)</small>` : ""}
            </div>
            <button type="button" class="disconnect-local-workbook" data-local-file-id="${escapeHtml(file.id)}">Desvincular</button>
        </div>`;
    }).join("");
    const urlHTML = urlConnections.map(connection => `<div class="connected-item connected-item-url"><span>${connection.connected ? "🔗" : "⚠️"}</span><div><strong>${escapeHtml(EXCEL_PANEL_LABELS[connection.key])}</strong><small>${connection.connected ? `URL conectada${connection.lastSync ? ` • ${escapeHtml(connection.lastSync)}` : ""}` : "URL cadastrada, aguardando sincronização"}</small></div></div>`).join("");
    list.innerHTML = localHTML + urlHTML;
}

function inferEntryYear(entry) {
    const match = String(entry.fileName || "").match(/(?:19|20)\d{2}/);
    return match ? Number(match[0]) : (isoToDate(state?.filterEnd)?.getFullYear() || isoToDate(state?.filterStart)?.getFullYear() || new Date().getFullYear());
}

function dailyEntryToFullDates(entry) {
    const parsed = parseStatusSheet(entry.matrix);
    if (!parsed) return null;
    let year = inferEntryYear(entry);
    let previousMonth = null;
    const dateKeys = parsed.days.map(label => {
        const match = String(label).match(/^(\d{1,2})\/(\d{1,2})(?:\/(\d{2,4}))?$/);
        if (!match) return "";
        const day = Number(match[1]);
        const month = Number(match[2]);
        let itemYear = match[3] ? Number(match[3]) : year;
        if (itemYear < 100) itemYear += 2000;
        if (!match[3] && previousMonth === 12 && month === 1) itemYear = ++year;
        previousMonth = month;
        return `${itemYear}-${String(month).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
    });
...
function buildContinuousDateKeys(startDate, endDate) {
...
function formatISOForDisplay(iso) {
    ...
    return date ? `${String(date.getDate()).padStart(2,"0")}/${String(date.getMonth()+1).padStart(2,"0")}` : iso;
}

function mergeParsedDailySheets(key) {
    const parsedEntries = getImportedSheetEntries(key).map(entry => ({ entry, parsed: dailyEntryToFullDates(entry) })).filter(item => item.parsed);
    if (!parsedEntries.length) return null;
    const allDateKeys = [...new Set(parsedEntries.flatMap(item => item.parsed.dateKeys).filter(Boolean))].sort();
    const farms = new Map();
    parsedEntries.forEach(({ parsed }) => {
        parsed.rows.forEach(row => {
            const farmKey = normalizeName(row.farm);
            if (!farms.has(farmKey)) farms.set(farmKey, { farm: row.farm, statusByDate: {} });
            parsed.dateKeys.forEach((dateKey, index) => {
                if (!dateKey) return;
                farms.get(farmKey).statusByDate[dateKey] = normalizeStatus(row.statuses[index]);
            });
        });
    });
    return {
        dateKeys: allDateKeys,
        days: allDateKeys.map(formatISOForDisplay),
        rows: [...farms.values()].map(farm => ({ farm: farm.farm, statusByDate: farm.statusByDate }))
    };
}

function applyDailySheet(key, parsed, selectedDateKeys, unmatched) {
    if (!parsed || !selectedDateKeys.length) return false;
    state.data[key] = {};
    state.farms.forEach((farm, farmIndex) => {
        state.data[key][farmIndex] = {};
        selectedDateKeys.forEach((dateKey, dayIndex) => {
            state.data[key][farmIndex][dayIndex] = "blank";
        });
    });
    parsed.rows.forEach(row => {
        const farmIndex = matchFarmIndex(row.farm);
        if (farmIndex === -1) { unmatched.add(row.farm); return; }
        selectedDateKeys.forEach((dateKey, dayIndex) => {
            state.data[key][farmIndex][dayIndex] = normalizeStatus(row.statusByDate?.[dateKey] ?? "blank");
        });
    });
    state.dailyDataReady = state.dailyDataReady || {};
    state.dailyDataReady[key] = true;
    state.manualEntryEnabled = false;
    return true;
}

function clearAllDerivedPanels() {
    ["campo","abastecimento","diario"].forEach(clearDailyPanel);
    clearMonthlyPanel();
    clearDivergencePanel();
    state.days = [];
    state.dayKeys = [];
...
function preencherSistema(options = {}) {
    ...
    if (!importedKeys.length) {
        ...
        if (save && typeof saveState === "function") saveState();
        const result = { found: [], failed: [], unmatched: [], noDatesInFilter: true };
        if (!silent) showInlineWarning("Nenhuma fonte ativa foi encontrada. Os dados derivados foram limpos.");
        return result;
    }
    const parsed = {
        campo: hasImportedData("campo") ? mergeParsedDailySheets("campo") : null,
    ...
    const found = [], failed = [], unmatched = new Set();
    const dailyKeys = ["campo","abastecimento","diario"];
    const availableDateKeys = [...new Set(dailyKeys.flatMap(key => parsed[key]?.dateKeys || []))].sort();
    const { startDate, endDate } = getImportDateRange();
    const selectedDateKeys = startDate && endDate
        ? buildContinuousDateKeys(startDate, endDate)
        : availableDateKeys;
    const noDatesInFilter = Boolean(startDate && endDate && !availableDateKeys.some(key => selectedDateKeys.includes(key)));
    state.dayKeys = selectedDateKeys;
    state.days = selectedDateKeys.map(formatISOForDisplay);

    dailyKeys.forEach(key => {
        if (!hasImportedData(key)) { clearDailyPanel(key); return; }
        if (!parsed[key] || !selectedDateKeys.length) { clearDailyPanel(key); failed.push(key); return; }
        if (applyDailySheet(key, parsed[key], selectedDateKeys, unmatched)) found.push(key);
        else { clearDailyPanel(key); failed.push(key); }
    });
    if (hasImportedData("mensal")) {
        if (applyMonthlySheet(parsed.mensal, unmatched)) found.push("mensal");
        else { clearMonthlyPanel(); failed.push("mensal"); }
    } else clearMonthlyPanel();
    if (hasImportedData("divergencias")) {
        if (applyDivergSheet(parsed.divergencias, unmatched)) found.push("divergencias");
        else failed.push("divergencias");
    } else clearDivergencePanel();
    ensureData();
    if (render && typeof renderAll === "function") renderAll();
    if (save && typeof saveState === "function") saveState();
    const result = { found, failed, unmatched: [...unmatched], noDatesInFilter };
...
async function loadLocalWorkbooksIntoImportedData(options = {}) {
    const records = await readLocalWorkbookBuffers({
        activeOnly: true,
        purgeOrphans: options.purgeOrphans !== false
    });
    ...
    for (const record of records) {
        try {
            const workbook = XLSX.read(record.buffer, {
                type: "array",
                cellDates: true
            });
            ...
            const file = ensureImportedFilesState()
                .find(item => String(item.id) === String(record.id));

            if (file) {
                file.sheets = result.recognizedSheets;
                file.panels = [
                    ...new Set(
                        result.recognizedSheets.map(item => item.key)
                    )
                ];
                file.ignoredSheets = [
                    ...new Set([
                ...
                file.extractionErrors = result.errors || [];
                file.lastSync = new Date().toLocaleTimeString(
                    "pt-BR",
                    {
                        hour: "2-digit",
                        minute: "2-digit"
            ...
            console.warn(
                `Não foi possível restaurar ${record.fileName || "planilha"}:`,
                error
            );
        }
    }
...
async function importLocalExcelFiles(event) {
    ...
    for (const file of files) {
        try {
            ...
            const existing = ensureImportedFilesState().find(item => normalizeName(item.fileName) === normalizeName(file.name));
            const id = existing?.id || `local-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
            const result = processWorkbook(workbook, file.name, { source: "local", fileId: id });
            if (!result.recognized.length) {
                errors.push(`${file.name}: nenhuma aba autorizada produziu dados válidos.`);
                continue;
            }
            registerImportedLocalFile(file.name, result.recognizedSheets, [...result.ignored, ...result.hidden], { id, errors: result.errors });
            await saveLocalWorkbookBuffer({ id, fileName: file.name, lastModified: file.lastModified, size: file.size, buffer });
        } catch (error) {
            console.error(`Erro ao importar ${file.name}:`, error);
            errors.push(`${file.name}: ${error?.message || "erro desconhecido"}`);
    ...
    input.value = "";
    const result = await refreshAllImportedSources({ silent: true, save: true });
    renderImportedWorkbookStatus();
    showInlineWarning(buildImportMessage(result, errors));
}
...
async function disconnectImportedLocalFile(fileId) {
    ...
    if (index === -1) return;
    const file = files[index];
    if (!window.confirm(`Deseja desvincular a planilha ${file.fileName}? Os dados vindos exclusivamente desse arquivo serão removidos.`)) return;
    files.splice(index, 1);
    await deleteLocalWorkbookBuffer(file.id).catch(error => console.warn("Não foi possível remover o arquivo do cache:", error));
    await refreshAllImportedSources({ silent: true, save: true });
    renderImportedWorkbookStatus();
    showInlineWarning(`Planilha ${file.fileName} desvinculada e dados recalculados.`);
}
