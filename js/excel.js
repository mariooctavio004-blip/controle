...
async function readLocalWorkbookBuffers(options = {}) {
    ...
    const metadataById = new Map(
        files
            .filter(item => item && item.source === "local" && item.id)
            .map(item => [String(item.id), item])
    );

    /*
     * O IndexedDB é a fonte local persistente do arquivo. O metadata
     * salvo no state/Supabase pode chegar depois, ser sobrescrito por
     * outro navegador ou ainda estar em uma versão antiga. Por isso,
     * nunca apagamos nem desconectamos automaticamente um arquivo que
     * continua existindo no IndexedDB deste navegador.
     */
    rows.forEach(row => {
        if (!row?.id) return;
        ...
        if (!metadataById.has(id)) {
            const restored = {
                id: row.id,
                source: "local",
                fileName: String(row.fileName || "Planilha importada"),
                sheets: [],
                panels: [],
                ignoredSheets: [],
                extractionErrors: [],
                importedAt: row.savedAt || new Date().toISOString(),
                updatedAt: new Date().toISOString(),
                lastSync: ""
            };

            files.push(restored);
            metadataById.set(id, restored);
        }
    });

    /*
     * Metadados sem buffer não são apagados automaticamente. Eles podem
     * ter vindo do Supabase e pertencer a outro navegador. Apenas não são
     * processados neste dispositivo até que o arquivo seja importado aqui.
     */
