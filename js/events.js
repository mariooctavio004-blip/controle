...
async function refreshConnectedData(button = null) {
    ...
    try {
        if (typeof refreshAllWorkbooks === "function") await refreshAllWorkbooks({ silent: true });
        if (typeof refreshAllImportedSources === "function") await refreshAllImportedSources({ silent: true, save: true });
        else if (typeof reapplyImportedDataForCurrentFilter === "function") await reapplyImportedDataForCurrentFilter({ silent: true, save: true });
        if (typeof renderAttachStatus === "function") renderAttachStatus();
        showInlineWarning("Dados atualizados com sucesso.");
    } catch (error) {
        console.error("Erro ao atualizar os dados:", error);
        showInlineWarning(error?.message || "Não foi possível atualizar os dados.");
    } finally {
...
function bindRefreshAndFilterIntegration() {
    ...
    if (!button || button.dataset.boundFullRefresh === "true") return;
    button.dataset.boundFullRefresh = "true";
...
/* ============================================================
   ATUALIZAÇÃO CONSOLIDADA DAS FONTES — VERSÃO FINAL
============================================================ */
async function refreshConnectedData(button = null) {
    if (sourcesRefreshRunning) return;
    sourcesRefreshRunning = true;
    ...
    if (button) { button.disabled = true; button.textContent = "⏳ Atualizando..."; }
    try {
        if (typeof clearImportedData === "function") clearImportedData();
        ...
        const result = typeof preencherSistema === "function"
            ? preencherSistema({ silent: true, save: true, render: true })
            : null;
        if (typeof renderAttachStatus === "function") renderAttachStatus();
        if (typeof renderImportedWorkbookStatus === "function") renderImportedWorkbookStatus();
        showInlineWarning(result?.found?.length
            ? "Arquivos locais e URLs foram reprocessados com sucesso."
            : "Nenhuma fonte ativa com dados válidos foi encontrada.");
    } catch (error) {
        console.error("Erro ao atualizar os dados:", error);
        showInlineWarning(error?.message || "Não foi possível atualizar os dados.");
    } finally {
